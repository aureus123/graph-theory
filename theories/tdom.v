...
(* Version of the Domination Theory in Graphs, with TGraphs *)
...
Proof.
  ...
  apply/andP.
  split ; last first.
  by rewrite -cltg_clneigh cl_tg_sym.
  apply/bigcupP.
  elim=> x /andP [_ xinDminusv] winNx.
  move: xinDminusv.
  rewrite in_setD in_set1 => /andP [xnotv xinD].
  move: winNx.
  rewrite -cltg_clneigh cl_tg_sym => xdomw.
  ...
  by rewrite cl_tg_sym cltg_clneigh.
  move=> u uinD udomw.
  apply/eqP.
  move: wnotincup.
  apply: contraR => unotv.
...
Proof.
  ...
  by rewrite set1Ur.
  rewrite negb_imply.
  rewrite negbK tg_sym adjwv andbT.
  by rewrite set1Ul.
  (* first case: -> *)
...
Proof.
  ...
  apply/implyP=> winD.
  by move: (H3 w winD (cl_tg_refl w))->.
  rewrite negb_exists.
  apply/forallP=> y.
  rewrite negb_and.
